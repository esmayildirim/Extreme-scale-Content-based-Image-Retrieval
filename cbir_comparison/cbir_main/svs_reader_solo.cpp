                            ...
                            int level_factor)
{
    ...
    openslide_read_region(osr, buf, xcoord, ycoord, level_index, tile_width, tile_height);
 ...
 */
    ...
    fprintf(log_file, "%d\t%d\t%"PRId64"\tpix\t%"PRId64"\tpix\t%.2f\tsec\t%.2f\tsec\t%.2f\tsec\t%"PRId64"\tMB\n",my_rank, p, tilex, tiley, t_read, t_write, total,  (int64_t) (t_read_bytes*1.0)/1024/1024);
    fclose(log_file);
