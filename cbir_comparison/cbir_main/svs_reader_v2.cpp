...
static uint32_t * read_tile(openslide_t *osr,
                            int file_index,
                            char ** file_list,
                            int *prev_file_index,
                            int level_index,
...
static uint32_t * read_query_image(char * file_path, int64_t * width, int64_t *height )
{
    ...
    *height = image2.cols;
    ...
    // Mat image3(image2.rows, image2.cols,CV_8UC4,buffer);
...
int main(int argc, char* argv[]){
    ...
    //printf("Total tile number %"PRId64"\n", total_tile_number);
    ...
    //time variables
    double t_read, t_write, t_cbir;
    t_read = t_write = t_cbir = 0;
    double temp_start, temp_end;
    double read_write_rs_fs_sort_start, read_write_rs_fs_sort_end;
    
    int64_t t_read_bytes, t_write_bytes;
    t_read_bytes = t_write_bytes = 0 ;
        ...
        //start read and write
    {
        ...
        read_write_rs_fs_sort_start = MPI_Wtime();
        ...
        for(i=start_tile; i <= end_tile; i++ )
        {
            ...
            buffer = read_tile(osr,
                               ...
                               level_factor);
            temp_end = MPI_Wtime();
            t_read += temp_end - temp_start;
            t_read_bytes += tile_width * tile_height * 4;
            
            
            //START ROUGH AND FINE SEARCHING
            temp_start =MPI_Wtime();
          
            vector<result_distance_t> patch_distance_vector;
            cbir_rs_fs_workflow( patch_distance_vector,
                                (unsigned char *)buffer, // ARGB tile buffer
                                tile_height,
                                tile_width,
                                (unsigned char *)query_image_buffer,//ARGB query image buffer
                                query_image_height,
                                query_image_width,
                                i, //global tile id
                                OLP, //sliding distance factor
                                KBINS, // number of rings
                                FEA_INDEX,
                                file_list[file_index],//svs file name
                                tilex_coordinate,
                                tiley_coordinate,
            ...
            temp_end = MPI_Wtime();
            t_cbir += temp_end - temp_start;
            
            //WRITE THE TILE
            temp_start = MPI_Wtime();
            
           /* write_tile_as_ppm(osr,
                              buffer,
                              tile_width,
                              tile_height,
                              "./output/output-",
            ...
            temp_end = MPI_Wtime();
            */
            t_write += temp_end - temp_start;
            t_write_bytes += tile_width * tile_height * 4;
        ...
        read_write_rs_fs_sort_end = MPI_Wtime();
        if(osr!=NULL)
            openslide_close(osr);
    }
    ...
    if(my_rank ==0)
        printf("RESULTS ARE GATHERED AFTER ROUGH AND FINE SEARCHING\n");
    
    double external_sort_crop_start, external_sort_crop_end;
   
    external_sort_crop_start = MPI_Wtime();
    external_parallel_sort_multifile_MPIIO(my_rank,
                                           p,
                                           "./sorted",
                                           "sorted_fs_results.bin",
                                           &two_way_merge_based_on_distance);    //read spilled fine searching results file is sorted in the disk in parallel.
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    if(my_rank == 0)
    printf("RESULTS ARE SORTED BASED ON TILE ID\n");
    
    external_sort_crop_end = MPI_Wtime();
    ...
    //printf("%d: start %lld end %lld number of tiles %lld\n", my_rank, start_offset, end_offset, my_number_of_tiles);
    MPI_Barrier(MPI_COMM_WORLD);
    
    double time_cl_fs_start, time_cl_fs_end;
    
    time_cl_fs_start = MPI_Wtime();
    float Beta = 1.2;
    cbir_cluster_fs_workflow(start_offset,
                             end_offset,
                             my_number_of_tiles,
                             "sorted_top_10_tileID_fs_results.bin",
                             Beta,
    ...
    MPI_Barrier(MPI_COMM_WORLD);
   if(my_rank == 0)
       printf("CLUSTERING AND FINAL FINE SEARCHING COMPLETED\n");
    
    
    external_parallel_sort_multifile_MPIIO(my_rank,
                                           p,
                                           "./sorted",
                                           "final_sorted_fs_results.bin",
                                           &two_way_merge_based_on_distance);
    MPI_Barrier(MPI_COMM_WORLD);
    
    time_cl_fs_end = MPI_Wtime();
    
    write_image("final_sorted_fs_results.bin", query_image_width, query_image_height, my_rank, level_index);
