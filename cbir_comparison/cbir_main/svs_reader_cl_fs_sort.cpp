...
int main(int argc, char* argv[]){
    ...
    double total_sort_by_tile_ID_time = 0;
    double total_sort_by_distance_time = 0;
...
/*    file_list = get_file_list(path[0], &no_of_files);
   ...
   */
    ...
    sort_end = MPI_Wtime();
    total_sort_by_tile_ID_time = sort_end - sort_start; 
   ...
   sort_end = MPI_Wtime();
   total_sort_by_distance_time = sort_end - sort_start;
    MPI_Barrier(MPI_COMM_WORLD);

    
    t_end = MPI_Wtime();
    
    write_image("final_sorted_fs_results.bin", query_image_width, query_image_height, my_rank, level);
    ...
    if(my_rank == 0)
    {
    log_file = fopen("results-2.log","a+");
    //if(my_rank == 0)
    	fprintf(log_file, "Sort_by_TILE_ID\tCL_FS_TIME\tSort_by_distance\tTotal_time\n");
    ...
    fprintf(log_file, "%.2f\t%.2f\t%.2f\t%.2f\n", total_sort_by_tile_ID_time, total_cl_fs_time, total_sort_by_distance_time,total);
